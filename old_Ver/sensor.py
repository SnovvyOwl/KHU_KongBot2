...
class serialConnect:
    def __init__(self,port,baud):
        self.connect = serial.Serial(port,baud)
        self.data = ""
        self.splData = [0, 0, 0, 0, 0, 0, 0, 0, 0]
        self.fout = open('ahrs.txt', 'w')

    def read(self):
        out=self.connect.readline()
        timestamp=datetime.fromtimestamp(time.time())
        self.data=str(timestamp)+">> "+str(out)
        self.splData=str(out[1:-1]).split(',')
...
class Gyro:
    ...
    def gyro_out(self):
        ...
        self.gyroout=str(self.gyro_xout)+","+str(self.gyro_yout)+","+str(self.gyro_zout)
